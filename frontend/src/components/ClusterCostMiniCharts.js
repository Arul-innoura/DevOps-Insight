...
/** Pick up to `count` evenly-spaced ticks, always including first and last. */
function pickXTicks(arr, count = 5) {
    if (!arr.length) return [];
    if (arr.length <= count) return arr;
    const out = [arr[0]];
    const step = (arr.length - 1) / (count - 1);
    for (let i = 1; i < count - 1; i++) out.push(arr[Math.round(i * step)]);
...
export function LiveAreaChart({
    ...
    height = 150, granularity = "minute", valueLabel = "",
    valueFmt, title, currentValue, currentAllocated,
    fromMs, toMs,   // optional: clamp X axis to the requested time window
}) {
    const VP_W  = 720;
    ...
    const { allTs, minT, maxT, max, spanMs } = useMemo(() => {
        ...
        // If the caller passes fromMs/toMs (from the selected time window), use them
        // so the X axis spans the full requested period even when data is sparse.
        const lo = (fromMs != null && fromMs > 0) ? fromMs : (sorted[0] ?? 0);
        const hi = (toMs   != null && toMs   > 0) ? toMs   : (sorted[sorted.length - 1] ?? 0);
        return { allTs: sorted, minT: lo, maxT: hi, max: m * 1.15 || 1, spanMs: hi - lo };
    }, [points, allocatedPoints, fromMs, toMs]);

    // When a full window is given (fromMs/toMs), spread ticks evenly across it so the
    // axis is always fully labelled even when actual data only covers part of the range.
    const xTickMs = useMemo(() => {
        if (fromMs != null && toMs != null && toMs > fromMs) {
            const n = 5;
            return Array.from({ length: n }, (_, i) => fromMs + (i / (n - 1)) * (toMs - fromMs));
        }
        return pickXTicks(allTs);
    }, [allTs, fromMs, toMs]);
            ...
            <svg viewBox={`0 0 ${VP_W} ${height}`} width="100%"
                ...
                {yTicks(max).map((tv, i) => (
                    <g key={i}>
                        ...
                        <text x={padL - 5} y={yOf(tv) + 3.5} textAnchor="end" fontSize="9" fill="#64748b">
                            {fmtV(tv)}
                        </text>
                    </g>
                ))}
                ...
                {/* X-axis tick labels — first=start, last=end to stay within viewBox */}
                {xTickMs.map((ms, i) => (
                    <text key={i} x={xOf(ms)} y={padT + innerH + 14}
                          textAnchor={i === 0 ? "start" : i === xTickMs.length - 1 ? "end" : "middle"}
...
// ─── CostBarChart ──────────────────────────────────────────────────────────────
export function CostBarChart({ points = [], color = "#10b981", height = 160, granularity = "hour", valueLabel = "$/hr" }) {
    const VP_W = 720;
    const padL = 58, padR = 12, padT = 12, padB = 40;
    const innerW = VP_W - padL - padR;
    const innerH = height - padT - padB;
    const [hover, setHover] = useState(null);
    ...
    return (
        <div style={{ position: "relative" }}>
            <svg viewBox={`0 0 ${VP_W} ${height}`} width="100%" style={{ display: "block", overflow: "visible" }}>
                ...
                {pickXTicks(points.map((_, i) => i)).map((idx, pos, arr) => (
                    <text key={idx} x={padL + idx * stride + stride / 2} y={padT + innerH + 14}
                          textAnchor={pos === 0 ? "start" : pos === arr.length - 1 ? "end" : "middle"}
                          fontSize="9" fill="#64748b">
                        {fmtTime(points[idx].t, granularity, spanMs)}
                    </text>
                ))}
            </svg>
            {hover && (
 ...
 */
export function CostMultiLineChart({ series = [], height = 160, granularity = "hour", valueFmt, valueLabel = "/hr", fromMs, toMs }) {
    const VP_W = 720;
    const padL = 58, padR = 12, padT = 12, padB = 40;
    const innerW = VP_W - padL - padR;
    const innerH = height - padT - padB;
