...
export default function CcCostHistoryPage({ ctx }) {
    ...
    // Total spend for the selected namespace over the loaded time window
    const nsRangeTotal = useMemo(() => {
        if (selectedNs === "__all__" || tsPoints.length < 2) return null;
        const vals = tsPoints.map(p =>
            (p.namespaces || []).find(n => n.namespace === selectedNs)?.hourlyUsd || 0
        );
        const avg   = vals.reduce((s, v) => s + v, 0) / vals.length;
        const durHr = (new Date(tsPoints[tsPoints.length - 1].t) - new Date(tsPoints[0].t)) / 3600000;
        return avg * Math.max(durHr, 0);
    }, [selectedNs, tsPoints]);
            ...
            {!tsWindow ? (
                ...
                <>
                                    ...
                                    <span style={{ fontSize: 12, color: "#64748b", display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                                        ...
                                        {nsRangeTotal != null && (
                                            <span style={{
                                                ...
                                                <DollarSign size={10}/>
                                                Range total&nbsp;<strong>{fmt$(nsRangeTotal)}</strong>
                                            </span>
                                        )}
                                    </span>
                                )}
                    ...
                    <div className="pcp-ts-chart">
                        ...
                        <CostMultiLineChart series={splitSeries} granularity={effectiveGranularity} height={180}
                    ...
                    {tsTopProjectSeries.length > 0 && (
                        <div className="pcp-ts-chart">
                            ...
                            <CostMultiLineChart series={tsTopProjectSeries} granularity={effectiveGranularity} height={200}
                                valueFmt={v => fmt$(v * costMultiplier)} valueLabel={costUnit}
                                fromMs={tsFromMs} toMs={tsToMs}/>
                        </div>
                    )}
