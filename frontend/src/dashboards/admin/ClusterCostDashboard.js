...
// Env-prefix tokens that should be stripped when guessing a project group from a namespace name.
// e.g. "qa-frontend" → "FRONTEND", "prod-api-gateway" → "API-GATEWAY"
const ENV_PREFIX_TOKENS = new Set([
    "qa", "dev", "prod", "production", "stage", "staging", "uat", "sit",
    "pre", "preprod", "sandbox", "test", "perf", "hotfix",
]);
...
export default function ClusterCostDashboard({ embedded = false }) {
    ...
    const projectGroupKey = useCallback((ns) => {
        ...
        // Skip leading env-prefix token so "qa-frontend" → "FRONTEND"
        const start = parts.length > 1 && ENV_PREFIX_TOKENS.has(parts[0].toLowerCase()) ? 1 : 0;
        const name = parts.slice(start).join("-");
        return (name || ns.namespace).toUpperCase();
    }, []);
    ...
    const pullSnapshot = useCallback(async () => {
        ...
        setRefreshing(true);
        try {
            const [snap, met, envData] = await Promise.all([
                getPrometheusLive(env),
                getPrometheusMetrics(env),
                getPrometheusEnvs(),
            ]);
            // Refresh the env dropdown so newly-added envs appear automatically
            if (Array.isArray(envData?.envs) && envData.envs.length) setEnvs(envData.envs);
            setSnapshot((prev) => {
                if (!snap) return prev;
    ...
    // Re-fetch time-series when filter/window changes.
    // For "live" mode also re-fetch on every snapshot tick so the graph stays current.
    // For historical modes (date/month/year/range) the data doesn't change — only
    // re-fetch when the filter itself changes, not on every snapshot update.
    const tsFetchTrigger = tsMode === "live" ? snapshot?.capturedAt : null;
    useEffect(() => {
        ...
        return () => { cancelled = true; };
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [env, tsWindow, effectiveGranularity, tsFetchTrigger]);

    // ── build context object passed to all pages ──────────────────────────
    const tsFromMs = tsWindow?.from ? new Date(tsWindow.from).getTime() : undefined;
    const tsToMs   = tsWindow?.to   ? new Date(tsWindow.to).getTime()   : undefined;
    const ctx = useMemo(() => ({
...
.pcp-prod-down > td { opacity:0.72; background:#fef2f2 !important; }
.pcp-prod-down > td:first-child { border-left:3px solid #dc2626; }
...
/* =====================================================
   DARK THEME
   ===================================================== */
[data-theme="dark"] .ccd-header { background:var(--card-bg); border-color:var(--border-color); }
[data-theme="dark"] .ccd-header-icon,[data-theme="dark"] .ccd-header-title { color:var(--text-main); }
[data-theme="dark"] .ccd-header-sub { color:var(--text-sub); }
[data-theme="dark"] .ccd-badge.updating { background:rgba(96,165,250,0.15); color:#60a5fa; animation:pcp-pulse 1.6s ease-in-out infinite; }
[data-theme="dark"] .ccd-badge.stale    { background:rgba(251,191,36,0.15); color:#fbbf24; border-color:rgba(251,191,36,0.3); }

[data-theme="dark"] .ccd-nav { background:var(--surface-subtle); border-color:var(--border-color); }
[data-theme="dark"] .ccd-nav-btn { color:var(--text-sub); }
[data-theme="dark"] .ccd-nav-btn:hover { background:var(--surface-muted); color:var(--text-main); }
[data-theme="dark"] .ccd-nav-btn.active { color:var(--text-main); }

[data-theme="dark"] .pcp-section { background:var(--card-bg); border-color:var(--border-color); }
[data-theme="dark"] .pcp-section-title { color:var(--text-main); }
[data-theme="dark"] .pcp-section-sub { color:var(--text-sub); }

[data-theme="dark"] .pcp-stat { background:var(--card-bg); border-color:var(--border-color); }
[data-theme="dark"] .pcp-stat.green { background:rgba(6,78,59,0.2);    border-color:rgba(52,211,153,0.25); }
[data-theme="dark"] .pcp-stat.amber { background:rgba(120,53,15,0.2);  border-color:rgba(251,191,36,0.25); }
[data-theme="dark"] .pcp-stat.blue  { background:rgba(30,58,138,0.2);  border-color:rgba(96,165,250,0.25); }
[data-theme="dark"] .pcp-stat-label { color:var(--text-muted); }
[data-theme="dark"] .pcp-stat-value { color:var(--text-main); }

[data-theme="dark"] .pcp-table th { color:var(--text-sub); border-color:var(--border-color); background:var(--surface-subtle); }
[data-theme="dark"] .pcp-table td { border-color:var(--border-light); color:var(--text-main); }
[data-theme="dark"] .pcp-table td.warn { color:#fbbf24; font-weight:600; }
[data-theme="dark"] .pcp-tot td { background:var(--surface-subtle); border-color:var(--border-color); }
[data-theme="dark"] .pcp-th-accent { background:rgba(30,58,138,0.3) !important; color:#93c5fd !important; }
[data-theme="dark"] .pcp-td-accent { background:rgba(30,58,138,0.18); color:#93c5fd; }
[data-theme="dark"] .pcp-uom { color:var(--text-muted); }

[data-theme="dark"] .pcp-pill { background:rgba(148,163,184,0.12); color:var(--text-sub); }
[data-theme="dark"] .pcp-pill.linked { background:rgba(96,165,250,0.15); color:#60a5fa; }
[data-theme="dark"] .pcp-pill.ok     { background:rgba(52,211,153,0.15); color:#34d399; }
[data-theme="dark"] .pcp-pill.warn   { background:rgba(251,191,36,0.15); color:#fbbf24; }
[data-theme="dark"] .pcp-pill.bad    { background:rgba(248,113,113,0.15); color:#f87171; }
[data-theme="dark"] .pcp-pill.cat-compute        { background:rgba(99,102,241,0.15);  color:#a5b4fc; }
[data-theme="dark"] .pcp-pill.cat-memory         { background:rgba(236,72,153,0.15);  color:#f9a8d4; }
[data-theme="dark"] .pcp-pill.cat-storage        { background:rgba(251,191,36,0.15);  color:#fbbf24; }
[data-theme="dark"] .pcp-pill.cat-network        { background:rgba(52,211,153,0.15);  color:#6ee7b7; }
[data-theme="dark"] .pcp-pill.cat-registry       { background:rgba(167,139,250,0.15); color:#c4b5fd; }
[data-theme="dark"] .pcp-pill.cat-database       { background:rgba(34,211,238,0.15);  color:#67e8f9; }
[data-theme="dark"] .pcp-pill.cat-other          { background:rgba(148,163,184,0.12); color:var(--text-sub); }
[data-theme="dark"] .pcp-pill.cat-system         { background:rgba(99,102,241,0.15);  color:#a5b4fc; }
[data-theme="dark"] .pcp-pill.cat-support        { background:rgba(52,211,153,0.12);  color:#6ee7b7; border-color:rgba(52,211,153,0.2); }
[data-theme="dark"] .pcp-pill.cat-control-plane  { background:rgba(99,102,241,0.15);  color:#a5b4fc; }
[data-theme="dark"] .pcp-pill.cat-system-vms     { background:rgba(99,102,241,0.12);  color:#c4b5fd; }
[data-theme="dark"] .pcp-pill.cat-system-osdisks { background:rgba(167,139,250,0.12); color:#c4b5fd; }
[data-theme="dark"] .pcp-pill.cat-user-allocated { background:rgba(52,211,153,0.15);  color:#6ee7b7; }
[data-theme="dark"] .pcp-pill.cat-user-vms       { background:rgba(52,211,153,0.12);  color:#86efac; }
[data-theme="dark"] .pcp-pill.cat-user-osdisks   { background:rgba(74,222,128,0.1);   color:#86efac; }
[data-theme="dark"] .pcp-pill.cat-spot-vms       { background:rgba(251,146,60,0.15);  color:#fb923c; }
[data-theme="dark"] .pcp-pill.cat-spot-osdisks   { background:rgba(253,186,116,0.12); color:#fdba74; }
[data-theme="dark"] .pcp-pill.cat-user-wastage   { background:rgba(248,113,113,0.12); color:#fca5a5; }
[data-theme="dark"] .pcp-pill.cat-egress         { background:rgba(253,186,116,0.12); color:#fed7aa; }

[data-theme="dark"] .pcp-mute   { color:var(--text-muted); }
[data-theme="dark"] .pcp-detail { color:var(--text-sub); }
[data-theme="dark"] .pcp-warn   { background:rgba(120,53,15,0.25); border-color:rgba(251,191,36,0.3); color:#fbbf24; }
[data-theme="dark"] .pcp-empty  { color:var(--text-sub); background:var(--surface-subtle); border-color:var(--border-color); }
[data-theme="dark"] .pcp-mode   { background:var(--card-bg); border-color:var(--border-color); color:var(--text-sub); }
[data-theme="dark"] .pcp-mode.on { background:rgba(96,165,250,0.15); border-color:rgba(96,165,250,0.3); color:#60a5fa; }
[data-theme="dark"] .pcp-env    { border-color:var(--border-color); background:var(--card-bg); color:var(--text-main); }
[data-theme="dark"] .pcp-btn    { border-color:var(--border-color); background:var(--card-bg); color:var(--text-sub); }
[data-theme="dark"] .pcp-btn-active { background:rgba(96,165,250,0.15); border-color:rgba(96,165,250,0.3); color:#60a5fa; }

[data-theme="dark"] .pcp-cluster     { background:var(--card-bg); border-color:var(--border-color); }
[data-theme="dark"] .pcp-cluster-row { color:var(--text-sub); }
[data-theme="dark"] .pcp-sku         { background:var(--surface-muted); color:var(--text-sub); }

[data-theme="dark"] .pcp-ops        { background:var(--card-bg); border-color:var(--border-color); }
[data-theme="dark"] .pcp-ops-title  { color:var(--text-sub); }
[data-theme="dark"] .pcp-mini       { background:var(--surface-subtle); border-color:var(--border-color); }
[data-theme="dark"] .pcp-mini-label { color:var(--text-muted); }
[data-theme="dark"] .pcp-mini-value { color:var(--text-main); }
[data-theme="dark"] .pcp-mini.red   { background:rgba(127,29,29,0.22); border-color:rgba(248,113,113,0.22); }
[data-theme="dark"] .pcp-mini.red   .pcp-mini-value { color:#f87171; }
[data-theme="dark"] .pcp-mini.amber { background:rgba(120,53,15,0.22); border-color:rgba(251,191,36,0.22); }
[data-theme="dark"] .pcp-mini.amber .pcp-mini-value { color:#fbbf24; }
[data-theme="dark"] .pcp-mini.green { background:rgba(6,78,59,0.22);   border-color:rgba(52,211,153,0.22); }
[data-theme="dark"] .pcp-mini.green .pcp-mini-value { color:#34d399; }

[data-theme="dark"] .pcp-diag         { background:var(--card-bg); border-color:var(--border-color); }
[data-theme="dark"] .pcp-diag-row     { color:var(--text-sub); }
[data-theme="dark"] .pcp-diag-warn    { background:rgba(127,29,29,0.22); color:#f87171; border-color:rgba(248,113,113,0.22); }
[data-theme="dark"] .pcp-diag-info    { background:rgba(30,58,138,0.22); color:#60a5fa; border-color:rgba(96,165,250,0.22); }
[data-theme="dark"] .pcp-diag-warnings summary { color:var(--text-muted); }
[data-theme="dark"] .pcp-diag-warnings li      { color:var(--text-sub); }

[data-theme="dark"] .pcp-node-ns-chip         { background:var(--surface-muted); }
[data-theme="dark"] .pcp-node-ns-chip strong  { color:var(--text-main); }
[data-theme="dark"] .pcp-node-ns-chip span    { color:var(--text-muted); }
[data-theme="dark"] .pcp-pct { background:rgba(120,53,15,0.22); color:#fbbf24; }

[data-theme="dark"] .pcp-toplist       { background:var(--card-bg); border-color:var(--border-color); }
[data-theme="dark"] .pcp-toplist-title { color:var(--text-sub); }
[data-theme="dark"] .pcp-tl-ns         { color:var(--text-muted); }
[data-theme="dark"] .pcp-tl-val        { color:var(--text-main); }

[data-theme="dark"] .pcp-stackbar { box-shadow:inset 0 0 0 1px var(--border-color); }

[data-theme="dark"] .pcp-sec-system   { border-left-color:#a78bfa; background:rgba(139,92,246,0.08); }
[data-theme="dark"] .pcp-sec-projects { border-left-color:#34d399;  background:rgba(16,185,129,0.08); }
[data-theme="dark"] .pcp-sec-resource { border-left-color:#fb923c;  background:rgba(249,115,22,0.08); }
[data-theme="dark"] .pcp-sec-history  { border-left-color:#60a5fa;  background:rgba(59,130,246,0.08); }

[data-theme="dark"] .pcp-substack-title { color:var(--text-sub); }
[data-theme="dark"] .pcp-split-line     { color:var(--text-muted); }
[data-theme="dark"] .pcp-image          { color:var(--text-muted); }
[data-theme="dark"] .pcp-hpa       { background:rgba(34,211,238,0.12); color:#67e8f9; }
[data-theme="dark"] .pcp-vm-size   { background:var(--surface-muted); color:var(--text-sub); }
[data-theme="dark"] .pcp-spot-badge { background:rgba(251,191,36,0.12); color:#fbbf24; border-color:rgba(251,191,36,0.25); }
[data-theme="dark"] .pcp-meta      { color:var(--text-muted); }
[data-theme="dark"] .pcp-meta-cost .mtd { color:#fbbf24; }
[data-theme="dark"] .pcp-meta-cost .cum { color:#60a5fa; }

[data-theme="dark"] .pcp-inv-group      { background:var(--card-bg); border-color:var(--border-color); }
[data-theme="dark"] .pcp-inv-group-head { border-color:var(--border-light); }
[data-theme="dark"] .pcp-inv-subtotal   { color:var(--text-sub); }
[data-theme="dark"] .pcp-inv-grand      { background:rgba(6,78,59,0.2); border-color:rgba(52,211,153,0.22); color:#34d399; }

[data-theme="dark"] .pcp-audit         { border-color:rgba(96,165,250,0.25); background:rgba(30,58,138,0.15); }
[data-theme="dark"] .pcp-audit-formula { color:var(--text-sub); background:var(--surface-subtle); border-color:var(--border-color); }

[data-theme="dark"] .pcp-ns            { background:var(--surface-subtle); border-color:var(--border-color); }
[data-theme="dark"] .pcp-ns-head       { background:var(--surface-subtle); color:var(--text-main); }
[data-theme="dark"] .pcp-ns-head:hover { background:var(--surface-muted); }
[data-theme="dark"] .pcp-ns-body       { background:var(--card-bg); border-color:var(--border-color); }

[data-theme="dark"] .pcp-prod-open > td    { background:var(--surface-subtle); }
[data-theme="dark"] .pcp-prod-detail       { background:var(--surface-subtle); }
[data-theme="dark"] .pcp-prod-down > td    { background:rgba(127,29,29,0.15) !important; }
[data-theme="dark"] .pcp-prod-down > td:first-child { border-left-color:#f87171; }

[data-theme="dark"] .pcp-proj-card         { background:var(--card-bg); border-color:rgba(52,211,153,0.22); }
[data-theme="dark"] .pcp-proj-name         { color:#34d399; }
[data-theme="dark"] .pcp-proj-ns-chip      { background:rgba(6,78,59,0.2); color:#6ee7b7; border-color:rgba(52,211,153,0.22); }
[data-theme="dark"] .pcp-proj-ns-chip strong { color:#34d399; }
[data-theme="dark"] .pcp-proj-hourly       { color:#34d399; }
[data-theme="dark"] .pcp-proj-hourly span  { color:var(--text-muted); }
[data-theme="dark"] .pcp-proj-monthly      { color:var(--text-main); }
[data-theme="dark"] .pcp-proj-monthly span { color:var(--text-muted); }
[data-theme="dark"] .pcp-proj-mtd          { color:#fbbf24; }
[data-theme="dark"] .pcp-proj-share        { color:var(--text-muted); }
[data-theme="dark"] .pcp-proj-bar          { background:rgba(52,211,153,0.12); }
[data-theme="dark"] .pcp-proj-bar-fill     { background:#10b981; }

[data-theme="dark"] .pcp-resource-card       { background:var(--card-bg); border-color:rgba(251,146,60,0.22); }
[data-theme="dark"] .pcp-resource-card-title { color:#fb923c; }

[data-theme="dark"] .pcp-ts-range-btn        { border-color:var(--border-color); background:var(--card-bg); color:var(--text-sub); }
[data-theme="dark"] .pcp-ts-range-btn:hover  { background:var(--surface-muted); }
[data-theme="dark"] .pcp-ts-range-btn.on     { background:rgba(96,165,250,0.15); border-color:rgba(96,165,250,0.3); color:#60a5fa; }
[data-theme="dark"] .pcp-ts-custom           { background:var(--card-bg); border-color:var(--border-color); color:var(--text-sub); }
[data-theme="dark"] .pcp-ts-custom input     { color:var(--text-main); }
[data-theme="dark"] .pcp-ts-mini-btn         { border-color:var(--border-color); background:var(--card-bg); color:var(--text-sub); }
[data-theme="dark"] .pcp-ts-mini-btn.on      { background:rgba(96,165,250,0.15); border-color:rgba(96,165,250,0.3); color:#60a5fa; }
[data-theme="dark"] .pcp-ts-gran             { color:var(--text-muted); }
[data-theme="dark"] .pcp-ts-gran select      { border-color:var(--border-color); background:var(--card-bg); color:var(--text-main); }
[data-theme="dark"] .pcp-ts-chart            { background:var(--card-bg); border-color:rgba(96,165,250,0.22); }
[data-theme="dark"] .pcp-ts-chart-title      { color:#60a5fa; }
[data-theme="dark"] .pcp-live-graph-wrap     { background:var(--card-bg); border-color:var(--border-color); }

[data-theme="dark"] .pcp-sec-total     { background:var(--card-bg); border-color:var(--border-color); color:var(--text-main); }
[data-theme="dark"] .pcp-sec-total-sub { color:var(--text-muted); }
`;
