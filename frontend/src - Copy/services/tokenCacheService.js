...
const TOKEN_BUFFER_MS = 60 * 1000; // Refresh 1 minute before expiry (maximises effective token lifetime)
...
const acquireToken = async (forceRefresh = false) => {
    try {
        ...
        if (active.idToken) {
            ...
            tokenExpiry = Date.now() + 7200000; // Assume 2 hour validity
...
const tokenCacheService = { getAuthToken, refreshAuthToken, clearTokenCache };
