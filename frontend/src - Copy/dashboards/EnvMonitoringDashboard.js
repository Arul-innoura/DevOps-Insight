            ...
            <div className="em-display-panel-head">
                <div>
                    ...
                    <p className="em-display-desc">
                        <strong>Show on chart</strong> controls the environment × product map below; daily bar charts
                        always keep full ticket history for every pair.
                        <strong> Live status:</strong> <strong>Running</strong> starts a manual uptime window from the
                        moment you click it until <strong>Stopped</strong> (saved to the server; charts and pills follow
                        that, not ticket polling). <strong>Auto</strong> uses open tickets only for Running / Stopped
                        and does not use the manual window.
                    </p>
                </div>
...
const EnvMonitoringDashboard = ({
    ...
    /** Pairs toggled "off" still appear on bar charts (history); this filter is only for the map legend. */
    const ticketsForShownOnChartMap = useMemo(
        () =>
            tickets.filter((t) => {
                const p = normalizedTicketProduct(t);
                const e = normalizedTicketEnv(t);
    ...
    const allProducts = useMemo(
        () => [...new Set(tickets.map((t) => normalizedTicketProduct(t)))].sort(),
        [tickets]
    );
    ...
    const chartNowMs = useMemo(
        () => Date.now(),
        [liveTick, dateFrom, dateTo, tickets, monitoringSettings]
    );

    const environments = useMemo(
        () => [...new Set(tickets.map((t) => normalizedTicketEnv(t)))].sort(),
        [tickets]
    );
    ...
    const productMetricsByEnv = useMemo(() => {
        ...
        environments.forEach((env) => {
            ...
            productsInEnv.forEach((p) => {
                m[p] = {
                    running: isProductRunningDisplayed(toggleMetaByPair, tickets, env, p),
        ...
        return out;
    }, [environments, tickets, toggleMetaByPair]);
        ...
        <div className="em-root">
                        ...
                        <div className="em-chart-card">
                            <div className="em-empty">
                                {isUser
                                    ...
                                    : 'No ticket data in this workspace yet.'}
                            </div>
                        </div>
                    ) : (
            ...
            {(activeTab === 'activity' || !showAdmin) && (
                <>
                    ...
                    <EnvProductMap tickets={ticketsForShownOnChartMap} productColors={productColors} />
                </>
            )}
