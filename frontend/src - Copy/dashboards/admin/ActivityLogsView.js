...
import { WS_MESSAGE_TYPES } from "../../services/stompWebSocketService";
...
const ActivityLogsView = () => {
    ...
    // Real-time: refresh on any ticket or devops event via WebSocket
    useRealTimeSync({
        onRefresh: () => fetchLogs(true),
        playNewTicketSound: false,
        playUpdateSound: false,
        eventTypes: [
        ...
        pollingInterval: 30_000, // fallback if WebSocket is unavailable
    });
    ...
    return (
        <div className="activity-logs-view" style={{ padding: "0 0 2rem 0" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.75rem", color: "#6B7280", fontSize: "0.8rem" }}>
                ...
                {lastRefresh && <span>Last refreshed: {lastRefresh.toLocaleTimeString()} · real-time via WebSocket</span>}
            </div>
