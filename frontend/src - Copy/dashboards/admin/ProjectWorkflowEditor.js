...
import { getCloudEnvironments } from "../../services/cloudEnvironmentService";
import WorkflowPersonSuggest from "../../components/WorkflowPersonSuggest";
import AzureSkuPicker from "../../components/AzureSkuPicker";
import AutoBuildSection from "./AutoBuildSection";
import ProjectCloudServicesPanel from "./ProjectCloudServicesPanel";
...
/** Standard Kubernetes CPU request presets (millicores). */
const CPU_PRESETS = [
    { label: "0.1 core  (100m)",  mCPU: 100  },
    { label: "0.25 core (250m)",  mCPU: 250  },
    { label: "0.5 core  (500m)",  mCPU: 500  },
    { label: "0.75 core (750m)",  mCPU: 750  },
    { label: "1 core    (1000m)", mCPU: 1000 },
    { label: "1.5 cores (1500m)", mCPU: 1500 },
    { label: "2 cores   (2000m)", mCPU: 2000 },
    { label: "4 cores   (4000m)", mCPU: 4000 },
    { label: "8 cores   (8000m)", mCPU: 8000 },
];

/** Standard Kubernetes memory request presets (megabytes). */
const RAM_PRESETS = [
    { label: "128 MB",  mb: 128   },
    { label: "256 MB",  mb: 256   },
    { label: "512 MB",  mb: 512   },
...
// ─── Services Section ─────────────────────────────────────────────────────────
const ServicesSection = ({ projectServices, setProjectServices, cloudServices, setCloudServices, availableEnvs = [] }) => {
    const [cloudSearch, setCloudSearch] = useState('');
    const [cloudResults, setCloudResults] = useState([]);
    const [servicesTab, setServicesTab] = useState("cloud");
    const [azurePickerFor, setAzurePickerFor] = useState(null);
    const [azurePriceUnit, setAzurePriceUnit] = useState("monthly"); // "hourly" | "daily" | "monthly"
    const [estimateEnvId, setEstimateEnvId] = useState("");
    const [estimatePoolKey, setEstimatePoolKey] = useState("userNodePool");
    ...
    const addProjectService = () => {
        setProjectServices((prev) => [
            ...prev,
            {
                ...
                serviceName: '', cpu: '', ram: '', notes: '',
                replicas: 1, cpuRequestMillicores: null, memoryRequestMb: null,
            }
        ]);
    };
    ...
    /** Stamp live Azure Retail Pricing row onto a cloud service. */
    const applyAzurePrice = (id, row) => {
        if (!row) return;
        const unit = (row.unitOfMeasure || "").toLowerCase();
        let hourly = row.retailPrice ?? null;
        if (hourly != null) {
            if (unit.includes("month")) hourly = row.retailPrice / 730.0;
            else if (unit.includes("day")) hourly = row.retailPrice / 24.0;
            else if (unit.includes("year")) hourly = row.retailPrice / (730.0 * 12.0);
        }
        const monthly = hourly != null ? hourly * 730.0 : null;
        setCloudServices((prev) => prev.map((s) => s.id === id ? ({
            ...s,
            azureMeterId: row.meterId,
            azureSkuName: row.skuName,
            azureProductName: row.productName,
            azureServiceName: row.serviceName,
            azureServiceFamily: row.serviceFamily,
            azureArmRegionName: row.armRegionName,
            azureUnitOfMeasure: row.unitOfMeasure,
            azureRetailPriceUsd: row.retailPrice,
            hourlyRateUsd: hourly,
            monthlyRateUsd: monthly,
            lastPriceFetchedAt: new Date().toISOString(),
            // Populate the existing price/currency fields for display
            currency: "USD",
            price: monthly != null ? monthly.toFixed(2) : (s.price || ""),
        }) : s));
    };
    ...
    // ── Cost estimate for microservices based on node pool capacity share ────
    const costEstimate = useMemo(() => {
        const env = availableEnvs.find(e => e.id === estimateEnvId);
        if (!env) return null;
        let pool;
        if (estimatePoolKey === "systemNodePool") pool = env.systemNodePool;
        else if (estimatePoolKey === "userNodePool") pool = env.userNodePool;
        else pool = (env.additionalNodePools || []).find(p => p.poolName === estimatePoolKey);
        if (!pool?.hourlyRateUsd || !pool?.nodeCount || !pool?.vCpuPerNode || !pool?.memoryGbPerNode) return null;

        const poolMonthlyCost = pool.hourlyRateUsd * pool.nodeCount * 730;
        const totalVcpu      = pool.vCpuPerNode * pool.nodeCount;
        const totalRamGb     = pool.memoryGbPerNode * pool.nodeCount;

        let projectVcpu = 0, projectRamGb = 0;
        for (const svc of projectServices) {
            const r = svc.replicas || 1;
            projectVcpu  += (r * (svc.cpuRequestMillicores || 0)) / 1000;
            projectRamGb += (r * (svc.memoryRequestMb     || 0)) / 1024;
        }

        const cpuFrac = totalVcpu  > 0 ? Math.min(projectVcpu  / totalVcpu,  1) : 0;
        const ramFrac = totalRamGb > 0 ? Math.min(projectRamGb / totalRamGb, 1) : 0;
        const share   = (cpuFrac + ramFrac) / 2;
        return { pool, poolMonthlyCost, totalVcpu, totalRamGb, projectVcpu, projectRamGb, cpuFrac, ramFrac, share, monthlyEst: share * poolMonthlyCost };
    }, [availableEnvs, estimateEnvId, estimatePoolKey, projectServices]);
        ...
        <div className="wf-services-layout">
            ...
            <div className="wf-metrics-grid">
                <div className="wf-metric-card">
                    ...
                    <span className="wf-metric-label">Azure Resources</span>
                </div>
                <div className="wf-metric-card">
                    ...
                    <span className="wf-metric-label">Microservices</span>
                </div>
                <div className="wf-metric-card">
            ...
            <div className="workflow-services-tabs" style={{ marginBottom: '1.25rem' }}>
                <button type="button" className={`workflow-services-tab ${servicesTab === "cloud" ? "active" : ""}`} onClick={() => setServicesTab("cloud")}>
                    <Cloud size={14} style={{ marginRight: 5 }} /> Azure Resources
                ...
                <button type="button" className={`workflow-services-tab ${servicesTab === "project" ? "active" : ""}`} onClick={() => setServicesTab("project")}>
                    <Server size={14} style={{ marginRight: 5 }} /> Microservices
            ...
            <div className="svc-sub">
                <div className="svc-sub-header">
                    <div>
                        <span className="svc-sub-title"><Cloud size={15} /> Project-owned Azure Resources</span>
                        <span className="svc-sub-hint">Azure services this project owns and pays for directly (e.g. CosmosDB, Service Bus, Blob Storage, Azure SQL). Link live Azure pricing for cost tracking.</span>
                    </div>
                </div>
                <div style={{ background: "var(--info-bg, #eff6ff)", border: "1px solid var(--info-border, #bfdbfe)", borderRadius: 6, padding: "8px 12px", marginBottom: 10, fontSize: 12, color: "var(--info-text, #1d4ed8)" }}>
                    <strong>What belongs here:</strong> Azure services allocated exclusively to this project — a project-specific CosmosDB, Service Bus namespace, dedicated SQL database, Blob Storage account, etc.
                    These are tracked via start/stop cycles on the <em>Cost Monitoring</em> dashboard.<br />
                    <strong>What belongs in the Environment config instead:</strong> services shared by all projects in an environment (shared Redis, RabbitMQ, API Management gateway, a shared PostgreSQL cluster) — configure those as <em>Shared services</em> on the environment.
                </div>
                ...
                {cloudServices.length === 0 ? (
                            ...
                            <div key={svc.id} className="svc-cloud-card svc-cloud-card-enhanced">
                                <div className="svc-cloud-card-head">
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                        ...
                                        {(svc.cloudPlatform || '').toLowerCase() === 'azure' && (
                                            <button type="button"
                                                onClick={() => setAzurePickerFor(svc.id)}
                                                title={svc.azureMeterId ? `Linked: ${svc.azureSkuName || svc.azureMeterId}` : "Pick a live Azure price"}
                                                style={{
                                                    display: 'inline-flex', alignItems: 'center', gap: 4,
                                                    padding: '5px 10px', fontSize: 12,
                                                    borderRadius: 6, cursor: 'pointer',
                                                    background: svc.azureMeterId ? '#dbeafe' : '#fff7ed',
                                                    border: `1px solid ${svc.azureMeterId ? '#93c5fd' : '#fdba74'}`,
                                                    color: svc.azureMeterId ? '#1d4ed8' : '#c2410c',
                                                    fontWeight: 600,
                                                }}>
                                                <DollarSign size={12} />
                                                {svc.azureMeterId ? `Azure: ${svc.azureSkuName || 'live'}` : 'Set Azure price (live)'}
                                            </button>
                                        )}
                                ...
                                <div className="svc-fields svc-cloud-form">
                                    ...
                                    {(svc.azureMeterId || svc.hourlyRateUsd != null) && (() => {
                                        const hr = svc.hourlyRateUsd ?? 0;
                                        const daily = hr * 24;
                                        const monthly = hr * 730;
                                        const displayAmt = azurePriceUnit === 'hourly' ? hr
                                            : azurePriceUnit === 'daily' ? daily : monthly;
                                        const unitLabel = azurePriceUnit === 'hourly' ? '/hour'
                                            : azurePriceUnit === 'daily' ? '/day' : '/month (730h)';
                                        return (
                                        <div style={{
                                            padding: '8px 10px', borderRadius: 6, fontSize: 12,
                                            background: '#eff6ff', border: '1px solid #bfdbfe',
                                            color: '#1e40af', display: 'grid', gap: 6,
                                        }}>
                                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 6 }}>
                                                <div>
                                                    <strong>Live Azure price:</strong>&nbsp;
                                                    ${svc.azureRetailPriceUsd?.toFixed(6)} per {svc.azureUnitOfMeasure || 'unit'}
                                                    &nbsp;— {svc.azureProductName || svc.azureSkuName || '–'} ({svc.azureArmRegionName || '–'})
                                                </div>
                                                <div style={{ display: 'flex', gap: 4 }}>
                                                    {['hourly','daily','monthly'].map(u => (
                                                        <button key={u} type="button" onClick={() => setAzurePriceUnit(u)}
                                                            style={{ padding: '2px 8px', fontSize: 10, borderRadius: 4, cursor: 'pointer',
                                                                background: azurePriceUnit === u ? '#3b82f6' : 'transparent',
                                                                color: azurePriceUnit === u ? '#fff' : '#3b82f6',
                                            ...
                                            <div style={{ fontSize: 16, fontWeight: 700, color: '#1d4ed8' }}>
                                                USD ${displayAmt.toFixed(azurePriceUnit === 'hourly' ? 6 : 2)}
                                                <span style={{ fontSize: 11, fontWeight: 400, color: '#3b82f6', marginLeft: 4 }}>{unitLabel}</span>
                                            </div>
                                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 4, fontSize: 11 }}>
                                                <span>Hourly: <strong>${hr.toFixed(6)}</strong></span>
                                                <span>Daily: <strong>${daily.toFixed(4)}</strong></span>
                                                <span>Monthly: <strong>${monthly.toFixed(2)}</strong></span>
                                            </div>
                                            <div style={{ fontSize: 10, color: '#3b82f6' }}>
                                                Refreshes every hour via Azure Retail Pricing API.
                                                {svc.lastPriceFetchedAt && ` Last fetched ${new Date(svc.lastPriceFetchedAt).toLocaleString()}`}
                                            </div>
                                        </div>
                                        );
                                    })()}
                                    <div style={{
                                        ...
                                        <label style={{ display: 'inline-flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
                                            <input type="checkbox" checked={!!svc.sharedAcrossProjects}
                                                onChange={(e) => updateCloudService(svc.id, "sharedAcrossProjects", e.target.checked)} />
                                            <span>Shared across projects (cost split proportionally)</span>
                                        </label>
                                        {svc.sharedAcrossProjects && (
                                            <input type="text"
                                                placeholder="Comma-separated project IDs sharing this resource"
                                                value={(svc.sharedProjectIds || []).join(', ')}
                                                onChange={(e) => updateCloudService(svc.id, "sharedProjectIds",
                                                    e.target.value.split(',').map(s => s.trim()).filter(Boolean))}
                                                style={{ flex: 1, padding: '4px 8px', fontSize: 12, border: '1px solid var(--border-color)', borderRadius: 4 }} />
                                        )}
                                    </div>
                                    <div className="workflow-input-group">
                    ...
                    <AzureSkuPicker
                        open={!!azurePickerFor}
                        onClose={() => setAzurePickerFor(null)}
                        onPick={(row) => { applyAzurePrice(azurePickerFor, row); setAzurePickerFor(null); }}
                        defaultService={(() => {
                            const s = cloudServices.find(c => c.id === azurePickerFor);
                            if (!s) return "";
                            // Map catalog names to Azure service names
                            if ((s.name || '').toLowerCase().includes('kubernetes')) return 'Azure Kubernetes Service';
                            if ((s.name || '').toLowerCase().includes('virtual machine')) return 'Virtual Machines';
                            if ((s.name || '').toLowerCase().includes('container registry')) return 'Container Registry';
                            if ((s.name || '').toLowerCase().includes('key vault')) return 'Key Vault';
                            if ((s.name || '').toLowerCase().includes('load balancer')) return 'Load Balancer';
                            if ((s.name || '').toLowerCase().includes('storage')) return 'Storage';
                            return s.category || '';
                        })()}
                    />
            ...
            <div className="svc-sub">
                <div className="svc-sub-header">
                    <div>
                        <span className="svc-sub-title"><Server size={15} /> Microservices</span>
                        <span className="svc-sub-hint">Pods / services running on AKS nodes inside this project. CPU &amp; memory requests drive the capacity-based cost split on the Cost Management dashboard.</span>
                    </div>
                    <button type="button" className="svc-add-btn svc-add-btn-enhanced" onClick={addProjectService}>
                        <Plus size={16} /> <span>Add microservice</span>
                    </button>
                </div>
                <div style={{ background: "var(--info-bg, #eff6ff)", border: "1px solid var(--info-border, #bfdbfe)", borderRadius: 6, padding: "8px 12px", marginBottom: 10, fontSize: 12, color: "var(--info-text, #1d4ed8)" }}>
                    Set <strong>Replicas</strong>, <strong>CPU/replica</strong>, and <strong>Memory/replica</strong> from the dropdowns.
                    These drive the cost estimate below — your project's share = (your CPU requests ÷ pool total CPU + your RAM requests ÷ pool total RAM) ÷ 2.
                </div>

                {projectServices.length === 0 ? (
                    <div className="svc-empty">No microservices added yet. Click "Add microservice" to begin.</div>
                ) : (
                        ...
                        {projectServices.map((svc) => (
                            <div key={svc.id} className="svc-project-card svc-project-card-enhanced">
                                ...
                                <div className="svc-fields">
                                    ...
                                    <div className="svc-resource-row" style={{ gridTemplateColumns: '1fr 1fr 1fr' }}>
                                        <div className="workflow-input-group">
                                            <label>
                                                <span className="label-text">Replicas</span>
                                            </label>
                                            <input
                                                ...
                                                placeholder="1"
                                                value={svc.replicas ?? ""}
                                                onChange={(e) => updateProjectService(svc.id, "replicas", e.target.value === "" ? null : Number(e.target.value))}
                                            />
                                        </div>
                                        <div className="workflow-input-group">
                                            <label>
                                                <span className="label-text">CPU / replica</span>
                                                {svc.cpuRequestMillicores > 0 && (
                                                    <span className="label-hint">{((svc.cpuRequestMillicores * (svc.replicas || 1)) / 1000).toFixed(2)} cores total</span>
                                                )}
                                            </label>
                                            <select
                                                className="svc-field-input"
                                                value={svc.cpuRequestMillicores ?? ""}
                                                onChange={(e) => {
                                                    const mCPU = e.target.value === "" ? null : Number(e.target.value);
                                                    const preset = CPU_PRESETS.find(p => p.mCPU === mCPU);
                                                    setProjectServices(prev => prev.map(s => s.id === svc.id
                                                        ? { ...s, cpuRequestMillicores: mCPU, cpu: preset?.label || s.cpu }
                                                        : s));
                                                }}
                                            >
                                                <option value="">Select CPU…</option>
                                                {CPU_PRESETS.map(p => <option key={p.mCPU} value={p.mCPU}>{p.label}</option>)}
                                            </select>
                                        </div>
                                        <div className="workflow-input-group">
                                            <label>
                                                <span className="label-text">Memory / replica</span>
                                                {svc.memoryRequestMb > 0 && (
                                                    <span className="label-hint">{(svc.memoryRequestMb * (svc.replicas || 1) / 1024).toFixed(2)} GB total</span>
                                                )}
                                            </label>
                                            <select
                                                className="svc-field-input"
                                                value={svc.memoryRequestMb ?? ""}
                                                onChange={(e) => {
                                                    const mb = e.target.value === "" ? null : Number(e.target.value);
                                                    const preset = RAM_PRESETS.find(p => p.mb === mb);
                                                    setProjectServices(prev => prev.map(s => s.id === svc.id
                                                        ? { ...s, memoryRequestMb: mb, ram: preset?.label || s.ram }
                                                        : s));
                                                }}
                                            >
                                                <option value="">Select Memory…</option>
                                                {RAM_PRESETS.map(p => <option key={p.mb} value={p.mb}>{p.label}</option>)}
                                            </select>
                                        </div>
                                    </div>
                                    <div className="workflow-input-group">
                ...
                {/* ── Cost estimate panel ──────────────────────────────────── */}
                {projectServices.length > 0 && (
                    <div style={{ marginTop: 16, padding: '14px 16px', background: '#f0fdf4', border: '1px solid #86efac', borderRadius: 8 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                            <DollarSign size={15} style={{ color: '#15803d' }} />
                            <strong style={{ fontSize: 13, color: '#15803d' }}>Estimated Monthly Cost — node pool share</strong>
                        </div>
                        {availableEnvs.length === 0 ? (
                            <div style={{ fontSize: 12, color: '#166534' }}>
                                No environments configured yet. Add one in <strong>Admin → Environments</strong> to see cost estimates.
                            </div>
                        ) : (
                            <>
                                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 10 }}>
                                    <select
                                        ...
                                        value={estimateEnvId}
                                        onChange={e => { setEstimateEnvId(e.target.value); setEstimatePoolKey("userNodePool"); }}
                                    >
                                        <option value="">Select environment to estimate…</option>
                                        {availableEnvs.map(e => (
                                            <option key={e.id} value={e.id}>{e.displayName || e.name} ({e.azureRegion || 'unknown region'})</option>
                                        ))}
                                    </select>
                                    {estimateEnvId && (() => {
                                        const env = availableEnvs.find(e => e.id === estimateEnvId);
                                        if (!env) return null;
                                        const pools = [
                                            env.systemNodePool?.vmSize ? { key: 'systemNodePool', label: `System pool: ${env.systemNodePool.vmSize} ×${env.systemNodePool.nodeCount}` } : null,
                                            env.userNodePool?.vmSize   ? { key: 'userNodePool',   label: `User pool: ${env.userNodePool.vmSize} ×${env.userNodePool.nodeCount}` } : null,
                                            ...(env.additionalNodePools || []).filter(p => p.vmSize).map(p => ({
                                                key: p.poolName,
                                                label: `${p.poolName || 'Pool'} [${p.kind}]: ${p.vmSize} ×${p.nodeCount}`
                                            }))
                                        ].filter(Boolean);
                                        if (!pools.length) return <span style={{ fontSize: 12, color: '#92400e' }}>No pools configured on this environment.</span>;
                                        return (
                                            <select className="svc-field-input" style={{ flex: 1, minWidth: 200, fontSize: 12 }}
                                                value={estimatePoolKey} onChange={e => setEstimatePoolKey(e.target.value)}>
                                ...
                                {costEstimate ? (
                                    <>
                                        <div style={{ fontSize: 26, fontWeight: 800, color: '#15803d', marginBottom: 8 }}>
                                            ~${costEstimate.monthlyEst.toFixed(2)}
                                            <span style={{ fontSize: 13, fontWeight: 400, color: '#166534', marginLeft: 6 }}>/month</span>
                                        </div>
                                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5px 20px', fontSize: 12, color: '#166534' }}>
                                            <span>Node pool cost: <strong>${costEstimate.poolMonthlyCost.toFixed(2)}/mo</strong> ({costEstimate.pool.nodeCount} nodes × ${(costEstimate.pool.hourlyRateUsd||0).toFixed(4)}/hr)</span>
                                            <span>Pool capacity: <strong>{costEstimate.totalVcpu} vCPU · {costEstimate.totalRamGb} GB RAM</strong></span>
                                            <span>This project CPU: <strong>{costEstimate.projectVcpu.toFixed(2)} vCPU</strong> → {(costEstimate.cpuFrac * 100).toFixed(1)}% of pool</span>
                                            <span>This project RAM: <strong>{costEstimate.projectRamGb.toFixed(2)} GB</strong> → {(costEstimate.ramFrac * 100).toFixed(1)}% of pool</span>
                                            <span style={{ gridColumn: '1/-1' }}>
                                                Avg share = ({(costEstimate.cpuFrac*100).toFixed(1)}% + {(costEstimate.ramFrac*100).toFixed(1)}%) ÷ 2 = <strong>{(costEstimate.share*100).toFixed(1)}%</strong>
                                                &nbsp;→ <strong>${costEstimate.monthlyEst.toFixed(2)}/mo</strong>
                                            </span>
                                        </div>
                                    </>
                                ) : estimateEnvId ? (
                                    <div style={{ fontSize: 12, color: '#92400e' }}>
                                        Selected pool has no VM size or pricing. Configure the environment's node pool (VM size, node count, vCPU/node, RAM/node, hourly rate) first.
                                    </div>
                                ) : (
                                    <div style={{ fontSize: 12, color: '#166534' }}>
                                        Pick an environment above to see this project's estimated monthly cost share.
                                    </div>
                                )}
                            </>
                        )}
...
const ProjectWorkflowEditor = ({ project, onClose, onSaved }) => {
    ...
    /** Managed CloudEnvironment options — replaces the static preset list. */
    const [availableEnvs, setAvailableEnvs] = useState([]);

    // project-level services (admin only)
    const [projectServices, setProjectServices] = useState([]);
    const [cloudServices, setCloudServices] = useState([]);
    const [serviceUsages, setServiceUsages] = useState([]);     // toggled catalog services
    const [externalServices, setExternalServices] = useState([]); // manually-priced (Mongo, etc.)
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            setServiceUsages(Array.isArray(data.serviceUsages) ? data.serviceUsages : []);
            setExternalServices(Array.isArray(data.externalServices) ? data.externalServices : []);
        } catch (e) {
    ...
    useEffect(() => {
        ...
        getCloudEnvironments()
            .then((rows) => { if (!cancelled) setAvailableEnvs(Array.isArray(rows) ? rows : []); })
            .catch(() => { if (!cancelled) setAvailableEnvs([]); });
                    ...
                    {loading ? (
                        ...
                        <>
                            <div className="workflow-project-tabs">
                                ...
                                <button
                                    type="button"
                                    className={`workflow-project-tab ${projectConfigTab === "cloudUsage" ? "active" : ""}`}
                                    onClick={() => setProjectConfigTab("cloudUsage")}
                                    title="Toggle which managed Cloud Services this project uses"
                                >
                                    <Cloud size={14} style={{ marginRight: 6, verticalAlign: 'middle' }} />
                                    Cloud Usage
                                    {(serviceUsages.length + externalServices.length) > 0 && (
                                        <span className="env-tab-badge" style={{ marginLeft: 7 }}>{serviceUsages.length + externalServices.length}</span>
                                    )}
                                </button>
                                <button
                                    type="button"
                                    className={`workflow-project-tab ${projectConfigTab === "autoBuild" ? "active" : ""}`}
                                    onClick={() => setProjectConfigTab("autoBuild")}
                                    title="Configure Jenkins auto-build for code-cut requests"
                                >
                            ...
                            {projectConfigTab === "autoBuild" ? (
                                <AutoBuildSection
                                    projectId={project.id}
                                    environments={managedEnvs}
                                    projectServices={projectServices}
                                />
                            ) : projectConfigTab === "cloudUsage" ? (
                                <ProjectCloudServicesPanel
                                    serviceUsages={serviceUsages}
                                    setServiceUsages={setServiceUsages}
                                ...
                                <>
                                    ...
                                    <div className="workflow-env-manager">
                                        ...
                                        <div className="workflow-env-add-row">
                                            <span className="workflow-env-add-label">
                                                {availableEnvs.length > 0 ? "Attach managed environment" : "Add preset"}
                                            </span>
                                            <div className="workflow-env-catalog">
                                                {(availableEnvs.length > 0
                                                    ? availableEnvs.map((e) => e.name)
                                                    : ENVIRONMENTS
                                                ).map((env) => (
                                                    <button
                                                        key={env}
                                                        ...
                                                        title={
                                                            availableEnvs.find((e) => e.name === env)?.displayName
                                                            || availableEnvs.find((e) => e.name === env)?.azureRegion
                                                            || undefined
                                                        }
                                                    >
                                                        <Plus size={12} /> {env}
                                                ...
                                                {availableEnvs.length === 0 && (
                                                    <span style={{ fontSize: 12, color: "var(--text-muted)" }}>
                                                        No managed environments yet — create some under Admin → Environments.
                                                    </span>
                                                )}
                                            </div>
                                        </div>
