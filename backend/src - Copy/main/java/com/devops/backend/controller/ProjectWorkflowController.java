...
public class ProjectWorkflowController {
    ...
    public ResponseEntity<ProjectWorkflowSettings> saveWorkflow(
        ...
        projectWorkflowSettingsRepository.findByProjectId(projectId).ifPresent(existing -> {
            body.setId(existing.getId());
            // Preserve fields managed by AutoBuildConfigController.
            // The workflow editor only sends workflow-level fields; a full document
            // replace would wipe the Jenkins credentials and env auto-build configs.
            if (body.getJenkinsConnection() == null) {
                body.setJenkinsConnection(existing.getJenkinsConnection());
            }
            if (body.getAutoBuildConfig() == null || body.getAutoBuildConfig().isEmpty()) {
                body.setAutoBuildConfig(existing.getAutoBuildConfig());
            }
            // Also preserve cluster infrastructure (managed by EnvironmentsManager, not workflow editor).
            if (body.getClusterInfrastructure() == null || body.getClusterInfrastructure().isEmpty()) {
                body.setClusterInfrastructure(existing.getClusterInfrastructure());
            }
        });
