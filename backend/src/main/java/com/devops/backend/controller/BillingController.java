...
public class BillingController {
    ...
    /**
     * Cost reconciliation for one env — triggers a fresh tick and returns the
     * accounting invariant checks. allPass=true means every dollar is 100% attributed.
     */
    @GetMapping("/prometheus/{env}/reconcile")
    @PreAuthorize("hasAnyAuthority('APPROLE_Admin','APPROLE_DevOps')")
    public PrometheusLiveCostSnapshot.CostReconciliation prometheusReconcile(@PathVariable String env) {
        PrometheusLiveCostSnapshot snap = prometheusCost.tick(env);
        var rc = snap.getReconciliation();
        return rc != null ? rc : PrometheusLiveCostSnapshot.CostReconciliation.builder()
                .checks(List.of())
                .allPass(false)
                .passCount(0)
                .totalChecks(0)
                .summary("No reconciliation data — tick may have failed.")
                .build();
    }
